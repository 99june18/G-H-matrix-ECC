...
if __name__ == "__main__":
    ...
    for poly in polys:
        ...
        if cols is None:
            ...
            continue
        print("SUCCESS", cols)
        with open("H_indices.txt", "w") as f:
            f.write(" ".join(map(str, cols)) + "\n")

        H_sel = H_full[:, cols]
        with open("H_symbol.txt", "w") as f:
            for row in H_sel:
                f.write(" ".join("%04X" % int(x) for x in row) + "\n")

        lines = []
        for r in range(H_sel.shape[0]):
            for bit in reversed(range(SYM_BITS)):
                lines.append(" ".join(
                    str((int(H_sel[r, c]) >> bit) & 1)
                    for c in range(H_sel.shape[1])))
        with open("H_binary_32x288.txt", "w") as f:
            f.write("\n".join(lines) + "\n")
        sys.exit(0)
    print("All polynomials tried, none succeeded")